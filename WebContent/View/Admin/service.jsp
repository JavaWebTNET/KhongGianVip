...
<%
if(request.getAttribute("vtservice")!=null){
	@SuppressWarnings("unchecked")
	Vector<Service> vtns = (Vector<Service>) request.getAttribute("vtservice");
%>
<!-- ------Contai center main--- ---- -->
	...
	<div class="col-sm-12 center-right-child">
		...
		<div class=" col-sm-12 content-main">
			<div class=" col-sm-12 form-group  form-add_news">
				<label class="col-sm-offset-5 col-sm-3 col-xs-12 title-add-news">Thêm Dịch Vụ Mới</label>
				<form role="form" method="post"
								action="${ pageContext.request.contextPath}/admin/service/create" class="col-sm-12">
			...
			<table	class="col-sm-12  table table-bordered table-condensed  table-news">
					...
					<tr>
						...
						<td>
							<form role="form" method="post"
								action="${ pageContext.request.contextPath}/admin/service/update" class="form-edit_news">
								<input type="hidden" name="id_service" class="id_news"
									value="<%=vtns.get(i).getId_sv()%>">
								<label class="control-label td-title title-edit-news col-sm-2">Tiêu đề (*)</label>
								<div class="col-sm-10 title_news">
									<input type="text" class="input_title_news form-control"
										name="title_service" value="<%=vtns.get(i).getTitle_sv()%>">
								</div>
								<label class="control-label td-title title-edit-news col-sm-2">Nội dung (*)</label>
								...
								<div class="col-sm-10 area-content-news">
									<textarea class="conten_news form-control" name="content_service" id="area-content-news-<%=vtns.get(i).getId_sv()%>" name="content-news"><%=vtns.get(i).getContent_sv()%>
		    			  			</textarea>
								</div>
						...
						<td class="a-xoa"><a class="a-xoa-slider"
							href="${pageContext.request.contextPath}/admin/service/delete?id_sv=<%=vtns.get(i).getId_sv() %>">Xóa</a></td>
